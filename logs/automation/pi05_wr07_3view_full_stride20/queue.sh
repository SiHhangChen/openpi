...
CONFIG_NAME="pi05_membench_wr07_3view_full_stride20"
EXP_NAME="wr07_3view_full_s20_bs48_10k_g0123"
DATASET_ROOT="${DATASET_HOME}/wr07_200seeds_v061"
NORM_STATS="${OPENPI_ROOT}/assets/${CONFIG_NAME}/wr07_200seeds_v061/norm_stats.json"
...
AUTOMATION_DIR="${OPENPI_ROOT}/logs/automation/pi05_wr07_3view_full_stride20"
...
log "running CPU-only WR07 stride-20 contract and data preflight"
...
from openpi.training.data_loader import create_data_loader, create_torch_dataset
...
assert (config.batch_size, config.num_train_steps, config.seed) == (48, 10_000, 42)
...
assert data_config.sample_stride == 20
assert data_config.use_quantile_norm
assert type(data_config.model_transforms.inputs[2].tokenizer) is PaligemmaTokenizer
...
assert len(source) == 7_426

model = nnx.eval_shape(config.model.create, jax.random.key(0))
all_params = nnx.state(model, nnx.Param).flat_state()
frozen = nnx.state(model, nnx.All(nnx.Param, config.freeze_filter)).flat_state()
trainable = nnx.state(model, config.trainable_filter).flat_state()
assert len(frozen) == 0
assert len(trainable) == len(all_params) == 51
assert any("PaliGemma/img" in "/".join(map(str, path)) for path in trainable)

preflight_config = dataclasses.replace(config, batch_size=1, num_workers=0)
observation, actions = next(iter(create_data_loader(preflight_config, shuffle=False, num_batches=1)))
image_shapes = {key: tuple(value.shape) for key, value in observation.images.items()}
assert set(image_shapes) == {"base_0_rgb", "left_wrist_0_rgb", "right_wrist_0_rgb"}
assert all(shape == (1, 224, 224, 3) for shape in image_shapes.values())
assert tuple(observation.state.shape) == (1, 32)
assert tuple(actions.shape) == (1, 50, 32)
assert tuple(observation.tokenized_prompt.shape) == (1, 200)
assert int(np.asarray(observation.tokenized_prompt_mask).sum()) <= 200

print(
    f"PREFLIGHT_OK config={config.name} stride={data_config.sample_stride} samples={len(source)} "
    f"cameras={cameras} trainable={len(trainable)}/{len(all_params)} stats_sha256={expected_sha256}"
)
PY
...
log "global batch 48, 10000 updates, anchor stride 20, contiguous horizon 50, data parallel 4, FSDP 1"
